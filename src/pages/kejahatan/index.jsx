...
const User = ({ title }) => {
    ...
    const TableConfiguration = {
        columns: [
            {
                key: 'no_laporan', title: 'Nomor Laporan Polisi',
                onSort: sort => setparams(prev => ({ ...prev, sort })),
                render: (value, row) => <Link className="text-blue-400" to={`/kejahatan/${row?._id}`}>{value ?? "-"}</Link>
            },
            {
                key: 'tanggal_laporan', title: 'Tanggal Laporan Polisi',
                onSort: sort => setparams(prev => ({ ...prev, sort })),
                render: (value) => {
            ...
            {
                key: 'tanggal_pidana', title: 'Tanggal Terjadinya TIndak Pidana',
                onSort: sort => setparams(prev => ({ ...prev, sort })),
                render: (value) => {
                    const tanggal = new Date(value);
                    const hasil = format(tanggal, "EEEE, dd MMMM yyyy | hh:mm", { locale: id });
            ...
            {
                key: 'taksiran_kerugian', title: 'Taksiran Kerugian',
                onSort: sort => setparams(prev => ({ ...prev, sort })),
                render: (value) => value ?? "-"
            },
        ]
